...
class PageTitleMixin(object):
    ...
    def render_to_response(self, context):
        ...
        if self.request.session.get('user_2fa_enabled', False):
            print '%%%%%%%%%%%%%%%%'
            print 'user_2fa enabled checking'
            user_2fa_enabled = self.request.session['user_2fa_enabled']
        
        if user_2fa_enabled:
            print 'since user has enabled 2fa'
            if self.request.session.get('totp_valid', False):
                totp_val = self.request.session['totp_valid']
                print 'totp value is '
                print totp_val
                ...
                if totp_val:
                    ...
                    print 'totp value is falseeeeeee 1'
                    response = shortcuts.redirect('/dashboard/twofactor')
                    return response
            else:
                print 'top value is falseeeeeee 2'
                #messages.warning(self.request, 'Please correct the 2fa code error')
